...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        try {
            $query = "INSERT INTO class_students (class_id, first_name, last_name, middle_name, student_id) VALUES (:class_id, :first_name, :last_name, :middle_name, :student_id)";
    ...
    if ($action === 'update') {
        try {
            $query = "UPDATE class_students SET first_name = :first_name, last_name = :last_name, middle_name = :middle_name, student_id = :student_id WHERE id = :id";
            $stmt = $db->prepare($query);
